...
for i = 1:length(simulationResults)
    ...
    plot(timePoints/30.4, weights, 'x-', 'linewidth', 3)
end
...
for i = 1:length(simulationResults)
    ...
    legendValues{i} = simulationResults{i}.individual.name;
    plot(timePoints/30.4, fat./weights, 'x-', 'linewidth', 3)
end
...
function plotHeightEstimate(simulationResults, referenceData, simSettings)
    parameterSetAirMale = [0.450    1.486]; 
    parameterSetAirFeMale = [0.450    1.493]; 
    legendValues = [];
        
    hold all
    conversionEquation = @(leanMass) 10.^parameterSetAirMale(2) * leanMass.^parameterSetAirMale(1);
    

    for i = 1:length(simulationResults)
        timePoints = simulationResults{1}.timePoints/30.4;   
        leanWeight = (simulationResults{i}.weight-simulationResults{i}.fat)/1000;
        approxHeight1 = conversionEquation(leanWeight);
        legendValues{i} = simulationResults{i}.individual.name;
        plot(timePoints, approxHeight1, 'linewidth', 3)     
    end
    legendValues{i+1} = 'Reference';
