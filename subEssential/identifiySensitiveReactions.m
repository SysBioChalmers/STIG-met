...
%important for growth out of the cofactor mediated reactions
...
for i = 1:length(cofactorFile)
    ...
    affectedReactions{i} = cofactorReactions;
    allReactions = [allReactions; cofactorReactions];    
end
allReactions = unique(allReactions);
...
length(allReactions)
reactionsWithFlux = abs(FBASolution.x(allReactions))>threshold;
reactionsWithFlux = allReactions(reactionsWithFlux);
...
for i = 1:size(shadowPriceResult,1)
   ...
   for j = 1:length(affectedReactions)
       if ismember(reactionsWithGrowthEffect(i), affectedReactions{j})
            cofactorString = [cofactorString ' ' cofactorFile{j}];
       end
   end
    fprintf('\t%s', cofactorString)
